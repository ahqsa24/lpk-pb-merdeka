...
export const FolderManager: React.FC<FolderManagerProps> = ({ type, baseUrl }) => {
    ...
    if (loading) return <div className="p-8 text-center text-gray-500 dark:text-gray-400">Loading Content folders...</div>;

    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-200 capitalize">{type} Folders</h3>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredFolders.map(folder => (
                    <div key={folder.id} className="group relative bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl shadow-sm hover:shadow-md transition-all duration-200">
                        <Link href={`${baseUrl}/${folder.id}`} className="block p-6">
                            ...
                            <h4 className="font-semibold text-gray-900 dark:text-white truncate mb-1">{folder.name}</h4>
                            <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 h-10 mb-3">{folder.description || 'No description'}</p>
                ...
                {folders.length === 0 && (
                    <div className="col-span-full py-12 text-center border-2 border-dashed border-gray-200 dark:border-zinc-700 rounded-xl">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">No folders found. Create one to get started.</p>
                    </div>
                )}
            </div>
