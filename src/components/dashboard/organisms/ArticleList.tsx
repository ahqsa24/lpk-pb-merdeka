...
export const ArticleList: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Artikel & Berita Terbaru</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {articles.map((article) => (
                    <div key={article.id} className="group bg-white dark:bg-zinc-900 rounded-xl overflow-hidden border border-gray-100 dark:border-zinc-800 hover:shadow-lg transition-all duration-300 flex flex-col h-full">
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                            ...
                            {/* Read More Button (Mock link since we don't have public detail pages yet, or we can modal it? 
                                User asked for "card-card" display. I will assume just display for now, or link to #
                            ) */}
                            {/* Actually, I should probably implement a simple read mode or just show it here. 
                                Since user request is "article to dashboard user", maybe a detail view is needed? 
                                But let's stick to the card list first. 
                            */}
